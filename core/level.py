...
class Level:
    ...
    def __init__(self, screen):
        ...
        self.frame = copy.deepcopy(self.point[self.game_level])   # 当前关卡矩阵  *********
    ...
    def level(self, lev):
        self.game_level = lev
        self.frame = copy.deepcopy(self.point[self.game_level])
    ...
    def move_ele(self, direction, posi = None):
        ...
        before = self.get_before_val(direction, posi)
        # 对前一个位置赋值
        self.set_before_val(direction, posi)
        # 对原先的位置赋值
    ...
    # 对某一元素对应方向之前的元素赋值
    def set_before_val(self, direction, posi):
        """ 对某一元素对应方向之前的元素赋值 """
        now = self.frame[posi[0]][posi[1]]              # 当前值
        before = self.get_before_val(direction, posi)   # 前一个方格的值
        before_val = None
